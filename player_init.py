...
class Player:
    ...
    def __init__(self, name):
        self.name = name
        self.deck = [
            'Estate', 'Estate', 'Estate', 'Copper', 'Copper', 'Copper',
    ...
    def _buy_phase(self):
        ...
        while self.buys_left > 0:
            ...
            if buy.lower() == 'none':
                break
            elif buy.lower() in base_cards.card_map:
                if self.money < dominion.parse_card(buy.capitalize()).buy_cost:
                    print("You can't afford that card!")
                    continue
                elif self.gain_card(buy.capitalize()) is True:
                    self.buys_left -= 1
                    self.money -= dominion.parse_card(buy.capitalize()).buy_cost
                    assert self.money >= 0
    ...
    def gain_card(self, gained_card, zone=False):
        ...
        card = dominion.parse_card(gained_card)
        if card.supply == 0:
            print("The {} supply pile is empty!".format(gained_card))
            return
        card.supply -= 1
        if card.supply == 0:
            if card == base_cards.province:
                dominion.empty_supplies += 3
            else:
                dominion.empty_supplies += 1
        zone.append(gained_card)
        self.gained_this_turn.append(gained_card)
    ...
    def _shuffle(self):
        """Shuffles discard pile (if present) into deck and pops/returns top card."""
